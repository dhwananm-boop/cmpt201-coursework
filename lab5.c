 ...
 * next_block = block_at(address_of(newly_freed_block) +
 * size_of(newly_freed_block))
 *
 ...
 * EXAMPLE:
 *
 * Considering the memory layout from the image in the Lab: [ ... | m | z | n |
 * f | ... ]
 *
 * Let's assume block 'z' is the `newly_freed_block`.
 *
 * 1. The algorithm identifies 'm' as the `prev_block` and 'n' as the
 * `next_block`.
 *
